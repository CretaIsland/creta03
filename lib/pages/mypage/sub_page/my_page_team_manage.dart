...
class _MyPageTeamManageState extends State<MyPageTeamManage> {
  ...
  Widget profileImageComponent(TeamManager teamManager) {
    return Container(
      ...
      decoration: BoxDecoration(
          ...
          image: teamManager.currentTeam!.profileImgUrl == ''
              ? null
              : DecorationImage(
      ...
      child: Center(
        child: Stack(
          children: [
            teamManager.currentTeam!.profileImgUrl != ''
                ? const SizedBox()
                : Text(teamManager.currentTeam!.name.substring(0, 1),
            ...
            BTN.opacity_gray_i_l(
                ...
                onPressed: () async {
                  try {
                    ...
                    if (_pickedFile != null) {
                      ...
                      if (fileBytes.isNotEmpty) {
                        HycopFactory.storage!
                            .uploadFile(_pickedFile!.name, _pickedFile!.mimeType!, fileBytes,
                                fileUsage: "profile")
                            .then((fileModel) {
                          if (fileModel != null) {
                    ...
                    logger.fine('something wrong in my_page_team_manage >> $error');
                  }
                })
          ],
        ),
  ...
  Widget bannerImageComponent(TeamManager teamManager) {
    return Container(
      ...
      decoration: BoxDecoration(
          ...
          image: teamManager.currentTeam!.channelBannerImg == ''
              ? null
              : DecorationImage(
      ...
      child: Center(
        child: Stack(
          children: [
            teamManager.currentTeam!.channelBannerImg != ''
                ? const SizedBox()
                : Text('선택된 배경 이미지가 없습니다.', style: CretaFont.bodySmall),
            Container(
  ...
  Widget memberComponent(UserPropertyModel memberData, TeamManager teamManager,
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
              ...
              color: memberData.profileImgUrl == ''
                  ? Colors.primaries[Random().nextInt(Colors.primaries.length)]
                  : null,
              image: memberData.profileImgUrl != ''
                  ? DecorationImage(
                      image: Image.network(memberData.profileImgUrl).image, fit: BoxFit.cover)
                  : null),
          child: memberData.profileImgUrl != ''
              ? null
              : Center(
                  child: Text(memberData.nickname.toString().substring(0, 1),
                      style: const TextStyle(
                          fontFamily: 'Pretendard', fontSize: 12, color: Colors.white)),
                ),
        ),
        ...
        checkPermission(memberData.email, teamManager.currentTeam!) == 2
            ? IgnorePointer(
                ignoring: true,
            ...
            : CretaWidgetDropDown(
                ...
                defaultValue: checkPermission(memberData.email, teamManager.currentTeam!),
                onSelected: (value) {
                  teamManager.changePermission(memberData.email,
                      checkPermission(memberData.email, teamManager.currentTeam!), value);
                }),
        const SizedBox(width: 20.0),
        checkPermission(userPropertyManager.userPropertyModel!.email, teamManager.currentTeam!) ==
                    2 ||
                memberData.email == userPropertyManager.userPropertyModel!.email
            ? const SizedBox()
            : BTN.line_blue_t_m(
                text: '내보내기',
                onPressed: () {
                  teamManager.deleteTeamMember(memberData.email,
                      checkPermission(memberData.email, teamManager.currentTeam!));
                  teamManager.notify();
                })
      ],
    );
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                  image: DecorationImage(
                      image: Image.memory(bannerImgBytes).image, fit: BoxFit.cover)),
            ),
            ...
            Padding(
              ...
              child: BTN.fill_blue_t_m(
                  ...
                  onPressed: () {
                    HycopFactory.storage!
                        .uploadFile(_pickedFile!.name, _pickedFile!.mimeType!, bannerImgBytes,
                            fileUsage: "banner")
                        .then((fileModel) {
                      if (fileModel != null) {
  ...
  Widget addMemberPopUp(TeamManager teamManager, UserPropertyManager userPropertyManager) {
    return ChangeNotifierProvider.value(
        value: teamManager,
        child: Consumer<TeamManager>(builder: (context, model, child) {
          return CretaDialog(
              width: 364,
              height: 372,
              title: '팀원 추가',
                    ...
                    Row(
                      children: [
                        ...
                        BTN.line_blue_t_m(
                            ...
                            height: 32,
                            onPressed: () async {
                              UserPropertyModel? newMember =
                                  await userPropertyManager.emailToModel(scopeController.text);
                              if (newMember != null) {
                    ...
                    Container(
                      ...
                      child: SingleChildScrollView(
                        child: Column(
                          children: [
                            for (var member
                                in teamManager.teamMemberMap[teamManager.currentTeam!.mid]!) ...[
                              Row(
                                children: [
                                  Container(
                                    width: 16,
                                    ...
                                    decoration: BoxDecoration(
                                        ...
                                        color: member.profileImgUrl == ''
                                            ? Colors.primaries[
                                                Random().nextInt(Colors.primaries.length)]
                                            : Colors.white,
                                        image: member.profileImgUrl != ''
                                            ? DecorationImage(
                                                image: Image.network(member.profileImgUrl).image,
                                                fit: BoxFit.cover)
                                            : null),
                                    child: member.profileImgUrl == ''
                                        ? Center(
                                            child: Text(
  ...
  Widget build(BuildContext context) {
    return CretaAccountManager.getTeamList.isEmpty
                            ...
                            children: [
                              Row(
                                ...
                                children: [
                                  ...
                                  CretaWidgetDropDown(items: [
                                    for (var team in teamManager.teamModelList)
                                      Text(team.name, style: CretaFont.bodyMedium)
                                  ], defaultValue: 0, width: 140.0, onSelected: (value) {})
                                ],
                              ),
                              divideLine(
                                  topPadding: 22.0, bottomPadding: 32.0, width: widget.width * .7),
                              Padding(
                              ...
                              divideLine(
                                  topPadding: 32.0, bottomPadding: 32.0, width: widget.width * .7),
                              Padding(
                                padding: const EdgeInsets.only(left: 12.0),
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    ...
                                    const SizedBox(height: 16.0),
                                    for (var member in teamManager
                                        .teamMemberMap[teamManager.currentTeam!.mid]!) ...[
                                      const SizedBox(height: 16.0),
                                    ...
                                    BTN.fill_blue_t_m(
                                        ...
                                        onPressed: () {
                                          showDialog(
                                              context: context,
                                              builder: (context) =>
                                                  addMemberPopUp(teamManager, userPropertyManager));
                                        })
                                  ],
                                ),
                              ),
