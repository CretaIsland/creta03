          ...
          child: SingleChildScrollView(
            child: widget.width > 500 ? Padding(
              ...
              child: Column(
                ...
                children: [
                  Text("개인 정보", style: CretaFont.displaySmall.copyWith(fontWeight: FontWeight.w600)),
                  MyPageCommonWidget.divideLine(width: widget.width * .6, padding: const  EdgeInsets.only(top: 30, bottom: 50)),
                  Row(  
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      ...
                      Column(
                        ...
                        children: [
                          MyPageCommonWidget.profileImgComponent(
                            ...
                            editBtn: Center(
                              child: BTN.opacity_gray_i_l(
                                ...
                                onPressed: () async {
                                  try {
                                    _selectedImg = await ImagePicker().pickImage(source: ImageSource.gallery);
                                    if(_selectedImg != null) {
                                      _selectedImg!.readAsBytes().then((value) {
                                        setState(() {
                                          _selectedImgBytes = value;
                                        });
                                        HycopFactory.storage!.uploadFile(_selectedImg!.name, _selectedImg!.mimeType!, _selectedImgBytes!).then((value) {
                                          if(value != null) {
                                            userPropertyManager.userPropertyModel!.profileImgUrl = value.url;
                                          }
                                        });
                                      });
                                    }
                          ...
                          BTN.line_blue_t_m(text: "기본 이미지로 변경", onPressed: () {
                  ...
                  MyPageCommonWidget.divideLine(width: widget.width * .6, padding: const  EdgeInsets.only(top: 30, bottom: 40)),
                  Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const SizedBox(width: 12.0),
                        ...
                        children: [
                          SizedBox(
                            ...
                            child: TextField(
                              ...
                              onEditingComplete: () => userPropertyManager.userPropertyModel!.nickname = _nicknameController.text,
                            )
                          ),
                          const SizedBox(height: 30),
                          Text(userPropertyManager.userPropertyModel!.email, style: CretaFont.bodyMedium.copyWith(color: CretaColor.text.shade400)),
                          const SizedBox(height: 30),
                          Text("01012341234", style: CretaFont.bodyMedium.copyWith(color: CretaColor.text.shade400)),
                          const SizedBox(height: 26),
                          BTN.line_blue_t_m(height: 32, text: "비밀번호 변경", onPressed: () => showDialog(context: context, builder: (context) => ChangePwdPopUp.changePwdPopUp(context)))
                        ],
                      )
                    ],
                  ),
                  ...
                  BTN.fill_blue_t_el(text: '변경사항 저장', onPressed: () async {
