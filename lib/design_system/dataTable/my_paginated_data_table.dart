...
// import 'package:flutter/data_table.dart';
...
///
/// Data is read lazily from a [MyDataTableSource]. The widget is presented
/// as a [Card].
///
...
/// {@tool dartpad}
///
/// This sample shows how to display a [MyDataTable] with three columns: name,
/// age, and role. The columns are defined by three [MyDataColumn] objects. The
/// table contains three rows of data for three example users, the data for
/// which is defined by three [MyDataRow] objects.
///
...
class MyPaginatedDataTable extends StatefulWidget {
  ...
  /// be false.
  ///
  /// The [source] should be a long-lived [MyDataTableSource]. The same source
  /// should be provided each time a particular [MyPaginatedDataTable] widget is
  /// created; avoid creating a new [MyDataTableSource] with each new instance of
  /// the [MyPaginatedDataTable] widget unless the data table really is to now
  /// show entirely different data from a new source.
  ///
  /// Themed by [DataTableTheme]. [DataTableThemeData.decoration] is ignored.
  /// To modify the border or background color of the [MyPaginatedDataTable], use
  /// [CardTheme], since a [Card] wraps the inner [MyDataTable].
  ...
  ///
  /// See [MyDataTable.sortColumnIndex] for details.
  ///
  ...
  /// in ascending order.
  ///
  /// See [MyDataTable.sortAscending] for details.
  final bool sortAscending;

  /// Invoked when the user selects or unselects every row, using the
  /// checkbox in the heading row.
  ///
  /// See [MyDataTable.onSelectAll].
  ...
  final MyDataTableSource source;
...
class MyPaginatedDataTableState extends State<MyPaginatedDataTable> {
  ...
  final Map<int, MyDataRow?> _rows = <int, MyDataRow?>{};
  ...
  void reassemble() {
    ...
    // triggering a notification, leaving the MyPaginatedDataTable in an invalid
  ...
  MyDataRow _getBlankRowFor(int index) {
    return MyDataRow.byIndex(
      index: index,
      cells: widget.columns.map<MyDataCell>((MyDataColumn column) => MyDataCell.empty).toList(),
    );
  }

  MyDataRow _getProgressIndicatorRowFor(int index) {
    bool haveProgressIndicator = false;
    final List<MyDataCell> cells = widget.columns.map<MyDataCell>((MyDataColumn column) {
      if (!column.numeric) {
        haveProgressIndicator = true;
        return const MyDataCell(CircularProgressIndicator());
      }
      return MyDataCell.empty;
    }).toList();
    if (!haveProgressIndicator) {
      haveProgressIndicator = true;
      cells[0] = const MyDataCell(CircularProgressIndicator());
    }
    return MyDataRow.byIndex(
      index: index,
      cells: cells,
  ...
  List<MyDataRow> _getRows(int firstRowIndex, int rowsPerPage) {
    final List<MyDataRow> result = <MyDataRow>[];
  ...
  Widget build(BuildContext context) {
    // TO DO(ianh): This whole build function doesn't handle RTL yet.
    ...
    if (widget.onRowsPerPageChanged != null) {
      List<int> availableValues = smallestLagerRowsPerPages(widget.availableRowsPerPage, _rowCount);
      final List<Widget> availableRowsPerPage =
          availableValues.map<DropdownMenuItem<int>>((int value) {
        return DropdownMenuItem<int>(
          value: value,
          child: Text('$value'),
        );
      }).toList();
      // final List<Widget> availableRowsPerPage = widget.availableRowsPerPage.where((int value) {
      //   return value <= _rowCount || value == widget.rowsPerPage;
      ...
      footerWidgets.addAll(<Widget>[
        Container(
            width: 14.0), // to match trailing padding in case we overflow and end up scrolling
  ...
  List<int> smallestLagerRowsPerPages(List<int> list, int x) {
    list.sort();
    int? smallestLargerThanX;
    for (int i = 0; i < list.length; i++) {
      if (list[i] > x) {
        smallestLargerThanX = list[i];
        break;
      }
    }
    if (smallestLargerThanX == null) {
      return list;
    }
    return list.where((element) => element <= smallestLargerThanX!).toList();
  }
}
