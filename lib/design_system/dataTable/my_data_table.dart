...
class MyColumnInfo {
  ...
  static double totalWidth(List<MyColumnInfo> list) {
    double total = 0;
...
// late List<MyDataColumn> _columns;
// late List<MyDataRow> _rows;

/// Signature for [MyDataColumn.onSort] callback.
typedef MyDataColumnSortCallback = void Function(int columnIndex, bool ascending);
...
class MyDataRow {
  ...
  ///
  /// If a [MyDataCell] in the row has its [MyDataCell.onTap] callback defined,
  ...
  ///
  /// If a [MyDataCell] in the row has its [MyDataCell.onTap], [MyDataCell.onDoubleTap],
  /// [MyDataCell.onLongPress], [MyDataCell.onTapCancel] or [MyDataCell.onTapDown] callback defined,
  /// that callback behavior overrides the gesture behavior of the row for
  /// that particular cell.
  ...
  bool get _debugInteractive =>
      onSelectChanged != null || cells.any((MyDataCell cell) => cell._debugInteractive);
}
...
class MyDataCell {
  ...
  static const MyDataCell empty = MyDataCell(SizedBox.shrink());
  ...
  /// row (if [MyDataRow.onSelectChanged] is provided).
...
/// multiple pages.
///
/// ## Performance considerations when wrapping [MyDataTable] with [SingleChildScrollView]
///
/// Wrapping a [MyDataTable] with [SingleChildScrollView] is expensive as [SingleChildScrollView]
/// mounts and paints the entire [MyDataTable] even when only some rows are visible. If scrolling in
...
///
/// {@tool dartpad}
/// This sample shows how to display a [MyDataTable] with alternate colors per
/// row, and a custom color for when the row is selected.
///
/// ** See code in examples/api/lib/material/data_table/data_table.1.dart **
/// {@end-tool}
///
/// [MyDataTable] can be sorted on the basis of any column in [columns] in
...
class MyDataTable extends StatefulWidget {
  ...
  MyDataTable({
    ...
    this.clipBehavior = Clip.none,
        ...
        assert(!rows.any((MyDataRow row) => row.cells.length != columns.length),
  ...
  /// color.
  ///
  /// If [MyDataRow.onSelectChanged] or [MyDataRow.onLongPress] is null, the row's
  ...
  /// [MyDataRow.color].
  ///
  /// {@template flutter.material.MyDataTable.dataRowColor}
  /// ```dart
  /// MyDataTable(
  ...
  ///
  /// {@template flutter.material.MyDataTable.headingRowColor}
  /// ```dart
  /// MyDataTable(
  ///   columns: _columns,
  ///   rows: _rows,
  ...
  /// selectable. However, if [MyDataRow.onSelectChanged] is not set for any row,
  ...
  ///
  /// This can be used to clip the content within the border of the [MyDataTable].
  ///
...
class _MyDataTableState extends State<MyDataTable> {
  ...
  double? get dataRowHeight =>
      widget.dataRowMinHeight == widget.dataRowMaxHeight ? widget.dataRowMinHeight : null;

  bool get _debugInteractive {
    return widget.columns.any((MyDataColumn column) => column._debugInteractive) ||
        widget.rows.any((MyDataRow row) => row._debugInteractive);
  }

  bool _columnDragging = false;
  //bool _columnSizeOn = false;
  ...
  void _handleSelectAll(bool? checked, bool someChecked) {
    ...
    if (widget.onSelectAll != null) {
      ...
      for (final MyDataRow row in widget.rows) {
  ...
  Widget _buildHeadingCell({
    ...
    final double effectiveHeadingRowHeight = widget.headingRowHeight ??
        ...
        MyDataTable._headingRowHeight;

    Widget labelArea = Container(
      width: widget.columnInfo[index].width,
      decoration: BoxDecoration(
        //border: Border.all(color: Colors.grey), //skpark
        color: _columnDragging
            ? CretaColor.secondary.withOpacity(0.5)
            : widget.headingRowColor?.resolve(<WidgetState>{}) ?? Colors.transparent,
      ),
      //padding: padding,
      height: effectiveHeadingRowHeight,
      alignment: numeric ? Alignment.centerRight : AlignmentDirectional.center,
      child: AnimatedDefaultTextStyle(
        style: DefaultTextStyle.of(context).style.merge(effectiveHeadingTextStyle),
        softWrap: false,
        duration: MyDataTable._sortArrowAnimationDuration,
        child: label,
      ),
    );

    Widget moveHandleArea = MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: Draggable(
          ...
          onDragEnd: (details) {
            //print('onDragEnd offset.dx=${details.offset}');
            setState(() {
    ...
    Widget resizeHandleArea = MouseRegion(
      // onEnter: (event) {
      //   print('onEnter');
      //   setState(() {
      //     _columnSizeOn = true;
      ...
      child: GestureDetector(
        onPanUpdate: (DragUpdateDetails details) {
          setState(() {
            double width = widget.columnInfo[index].width + details.delta.dx;
            if (width < 5) {
              width = 5;
            }
            widget.columnInfo[index].width = width;
          });
    ...
    //         color: headingRowColor?.resolve(<WidgetState>{}) ?? Colors.transparent,
    //       ),
    //       padding: padding,
    //       height: effectiveHeadingRowHeight,
    ...
    // TO DO(dkwingsmt): Only wrap Inkwell if onSort != null. Blocked by
    // https://github.com/flutter/flutter/issues/51152
    //skpark

    // 컬럼이동과 컬럼 사이즈 변경이 서로 간섭하지 않도록 하기 위해
    // label = InkWell(
    //   onTap: onSort,
    //   overlayColor: overlayColor,
  ...
  int _getTargetIndex(double x) {
    int index = 0;
    double width = 0;
    for (int i = 0; i < widget.columnInfo.length; i++) {
      width += widget.columnInfo[i].width;
      if (x < width) {
        index = i;
        break;
      }
  ...
  Widget _buildDataCell({
    ...
    label = Container(
      ...
      constraints: BoxConstraints(
          minHeight: effectiveDataRowMinHeight, maxHeight: effectiveDataRowMaxHeight),
      alignment: numeric ? Alignment.centerRight : AlignmentDirectional.centerStart,
  ...
  Widget build(BuildContext context) {
    // for (var element in widget.columnInfo) {
    ...
    final WidgetStateProperty<Color?>? effectiveDataRowColor =
        widget.dataRowColor ?? dataTableTheme.dataRowColor ?? theme.dataTableTheme.dataRowColor;
    ...
    final bool anyRowSelectable = widget.rows.any((MyDataRow row) => row.onSelectChanged != null);
    final bool displayCheckboxColumn = widget.showCheckboxColumn && anyRowSelectable;
    final Iterable<MyDataRow> rowsWithCheckbox = displayCheckboxColumn
        ? widget.rows.where((MyDataRow row) => row.onSelectChanged != null)
        : <MyDataRow>[];
    final Iterable<MyDataRow> rowsChecked = rowsWithCheckbox.where((MyDataRow row) => row.selected);
    ...
    final List<TableRow> tableRows = List<TableRow>.generate(
      ...
      (int index) {
        ...
        final bool isDisabled =
            index > 0 && anyRowSelectable && widget.rows[index - 1].onSelectChanged == null;
        final Set<WidgetState> states = <WidgetState>{
          if (isSelected) WidgetState.selected,
          if (isDisabled) WidgetState.disabled,
        };
        final Color? resolvedDataRowColor = index > 0
            ? (widget.rows[index - 1].color ?? effectiveDataRowColor)?.resolve(states)
            : null;
        ...
        return TableRow(
          key: index == 0 ? MyDataTable._headingRowKey : widget.rows[index - 1].key,
          decoration: BoxDecoration(
            border: border,
    ...
    if (displayCheckboxColumn) {
      ...
      tableRows[0].children[0] = _buildCheckbox(
        ...
        onRowTap: null, //skpark
        onSelectedChanged: (bool? checked) => _handleSelectAll(checked, someChecked),
        overlayColor: null,
        tristate: true,
      );
      rowIndex = 1;
      for (final MyDataRow row in widget.rows) {
        ...
        tableRows[rowIndex].children[0] = _buildCheckbox(
          ...
          onSelectedChanged: row.onSelectChanged,
    ...
    for (int dataColumnIndex = 0; dataColumnIndex < widget.columns.length; dataColumnIndex += 1) {
      final MyDataColumn column = widget.columns[dataColumnIndex];

      final double paddingStart = switch (dataColumnIndex) {
        0 when displayCheckboxColumn && widget.checkboxHorizontalMargin == null =>
          effectiveHorizontalMargin / 2.0,
      ...
      tableRows[0].children[displayColumnIndex] = _buildHeadingCell(
        ...
        onSort: column.onSort != null
            ? () => column.onSort!(
                dataColumnIndex, widget.sortColumnIndex != dataColumnIndex || !widget.sortAscending)
            : null,
      ...
      for (final MyDataRow row in widget.rows) {
        ...
        final MyDataCell cell = row.cells[dataColumnIndex];
...
///
/// The [MyTableRowInkWell] must be in the same coordinate space (modulo
...
/// achieve: just put the [MyTableRowInkWell] as the direct child of the
/// [Table], and put the other contents of the cell inside it.)
///
/// See also:
///
///  * [MyDataTable], which makes use of [MyTableRowInkWell] when
///    [MyDataRow.onSelectChanged] is defined and [MyDataCell.onTap]
///    is not.
class MyTableRowInkWell extends InkResponse {
  /// Creates an ink well for a table row.
  const MyTableRowInkWell({
    super.key,
    super.child,
    super.onTap,
    super.onDoubleTap,
  ...
  RectCallback getRectCallback(RenderBox referenceBox) {
    return () {
      ...
      if (table is RenderTable) {
        ...
        // MyTableRowInkWell's coordinate space.
        table.applyPaintTransform(cell, transform);
...
class _SortArrowState extends State<_SortArrow> with TickerProviderStateMixin {
  ...
  void _resetOrientationAnimation(AnimationStatus status) {
    if (status == AnimationStatus.completed) {
      ...
      _orientationController.value = 0.0; // TO DO(ianh): This triggers a pointless rebuild.
    }
  }
