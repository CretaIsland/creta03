...
class CretaOvalBottomClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    var path = Path();
    path.lineTo(0, 0);
    path.lineTo(0, size.height - 3 * delta);
    path.quadraticBezierTo(size.width / 4, size.height, size.width / 2, size.height);
    path.quadraticBezierTo(
        size.width - size.width / 4, size.height, size.width, size.height - 3 * delta);
...
class CretaOvalRightClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    var path = Path();
    path.lineTo(0, 0);
    path.lineTo(size.width - 3 * delta, 0);
    path.quadraticBezierTo(size.width, size.height / 4, size.width, size.height / 2);
    path.quadraticBezierTo(
        size.width, size.height - (size.height / 4), size.width - 3 * delta, size.height);
    path.lineTo(0, size.height);
    return path;
  }
